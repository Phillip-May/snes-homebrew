...
def write_inc(path, arrays, defines=None, comment=None):
    ...
    for a in arrays:
        if a["kind"] == "u8":
            v = a["values"]
            parts.append("const unsigned char %s[%d] = {" % (a["name"], len(v)))
            parts.append(_rows(v, 16, "0x%02X"))
            parts.append("};")
        elif a["kind"] == "u16":
            v = a["values"]
            parts.append("const unsigned short %s[%d] = {" % (a["name"], len(v)))
            parts.append(_rows(v, 16, "0x%04X"))
            parts.append("};")
        elif a["kind"] == "u16_2d":
            rows = a["rows"]
            cols = len(rows[0]) if rows else 0
            parts.append("const unsigned short %s[%d][%d] = {"
                         % (a["name"], len(rows), cols))
            body = []
            for r in rows:
