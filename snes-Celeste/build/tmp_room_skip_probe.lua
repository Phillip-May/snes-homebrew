...
local outPath = "C:/Users/Admin/Documents/snes-homebrew/snes-Celeste/build/room_skip_probe_result.txt"
...
emu.addEventCallback(function()
    ...
    elseif room == 1 and frame >= 600 and frame < 720 then
        if ((frame - 600) % 20) == 0 then
            inputState.r = true
        end
    end
    ...
    if frame == 1 or (frame % 10) == 0 or room ~= 0 then
        log(string.format("f=%d fc=%d room=%d px=%d py=%d inputLo=%02X r=%s",
            frame, fc, room, px, py, inputLo, tostring(inputState.r)))
    end
