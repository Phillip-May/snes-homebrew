...
local outTxt = "C:/Users/Admin/Documents/snes-homebrew/snes-Celeste/build/monument_safe_verify_result.txt"
local outPng = "C:/Users/Admin/Documents/snes-homebrew/snes-Celeste/build/monument_safe_verify.png"
...
emu.addEventCallback(function()
    ...
    if room == TARGET_ROOM then
        if roomHitFrame == nil then
            roomHitFrame = frame
            log(string.format("ROOM12 frame=%d fc=%d", frame, frameCount))
        end
        ...
        if monumentStartFrame == nil and (monDisplayed ~= 0 or monLine ~= 0 or monChar ~= 0) then
            ...
            log(string.format("TEXT_START frame=%d fc=%d mon=%d line=%d char=%d tick=%d",
                frame, frameCount, monDisplayed, monLine, monChar, monTick))
        end
    end
    ...
    if frame == 1 or (frame % 30) == 0 or room == TARGET_ROOM then
        log(string.format("f=%d fc=%d room=%d same=%d px=%d py=%d mon=%d line=%d char=%d tick=%d",
            frame, frameCount, room, sameFrameCount, rd16(PLAYER_X), rd16(PLAYER_Y),
    ...
    if roomHitFrame ~= nil and room ~= TARGET_ROOM then
        log(string.format("LEFT_ROOM frame=%d fc=%d room=%d", frame, frameCount, room))
        done = true
        if emu.stop then
            emu.stop(1)
        end
