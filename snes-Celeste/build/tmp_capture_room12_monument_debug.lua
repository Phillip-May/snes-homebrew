...
local outTxt = "C:/Users/Admin/Documents/snes-homebrew/snes-Celeste/build/room12_monument_debug.txt"
...
local monumentX = nil
local monumentY = nil
local monumentIndex = nil
local textStartFrame = nil
...
emu.addEventCallback(function()
  ...
  local runCalls = rd16(ADDR_DBG_RUN)
  local refreshCalls = rd16(ADDR_DBG_REFRESH)
  local rawType = 0
  local rawFlags = 0
  if monumentIndex ~= nil then
    local base = ADDR_OBJLIST + (monumentIndex * OBJ_SIZE)
    rawFlags = rd8(base + 4)
    rawType = rd8(base + OBJ_TYPE_OFFSET)
  end
  ...
  if room == TARGET_ROOM then
    ...
    if textStartFrame == nil and monDisplayed ~= 0 then
      textStartFrame = frame
      log(string.format('TEXT_START frame=%d fc=%d char=%d calls=%d before=%d after=%d touch=%d flags=%d freeze=%d rawType=%d rawFlags=%d run=%d refresh=%d stage=%d objPass=%d objIndex=%d objType=%d objStep=%d', frame, fc, monChar, dbgCalls, dbgBefore, dbgAfter, dbgTouch, dbgFlags, freeze, rawType, rawFlags, runCalls, refreshCalls, dbgStage, dbgObjPass, dbgObjIndex, dbgObjType, dbgObjStep))
    end
  end

  if frame == 1 or room == TARGET_ROOM or (frame % 30) == 0 then
    log(string.format('f=%d fc=%d room=%d px=%d py=%d mon=%d line=%d char=%d tick=%d calls=%d before=%d after=%d touch=%d flags=%d freeze=%d rawType=%d rawFlags=%d run=%d refresh=%d stage=%d objPass=%d objIndex=%d objType=%d objStep=%d', frame, fc, room, rd16(PLAYER_X), rd16(PLAYER_Y), monDisplayed, monLine, monChar, monTick, dbgCalls, dbgBefore, dbgAfter, dbgTouch, dbgFlags, freeze, rawType, rawFlags, runCalls, refreshCalls, dbgStage, dbgObjPass, dbgObjIndex, dbgObjType, dbgObjStep))
  end

  if textStartFrame ~= nil and (frame - textStartFrame) >= 120 then
    log('DONE')
    done = true
