...
local out = "C:/Users/Admin/Documents/snes-homebrew/snes-Celeste/build/framecount_write_trace.txt"
...
if emu.addMemoryCallback then
    emu.addMemoryCallback(function(addr, val)
        ...
        if frame < 180 or frame > 240 then
            return
        end
        ...
        log(string.format(
            "WRITE f=%d room=%d addr=%06X val=%02X fc=%04X pc=%04X k=%02X sp=%04X a=%04X x=%04X y=%04X p=%02X",
            frame, room, addr, val, fc, pc, k, sp, a, x, y, p
        ))
        flush()
    end, emu.callbackType.write, ADDR_FRAME)
    ...
    emu.addMemoryCallback(function(addr, val)
        ...
        flush()
    end, emu.callbackType.write, ADDR_FRAME + 1)
end
