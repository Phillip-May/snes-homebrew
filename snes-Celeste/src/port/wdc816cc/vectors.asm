...
START:
		...
		;ldx     #<_ROM_BEG_DATA             ; Load source into X + 0x8000 (LoROM)
		ldx     #$8000
		ldy     #<_BEG_DATA                 ; Load dest into Y
		;HardCoded to bank 2 for data's initial values
		mvn     #$04,#^_BEG_DATA ; Copy bytes		
		SKIP:
		
		;lda     #_END_CEL_D_00-_BEG_CEL_D_00 ; Number of bytes to copy
		beq     SKIP2                        ; If none, just skip
		dec     a                           ; Decrement size for mvn
		
		;ldx     #<_ROM_BEG_CEL_D_00             ; Load source into X + 0x8000 (LoROM)
		;ldx     #$8000
		;ldy     #<_BEG_CEL_D_00                 ; Load dest into Y
		;HardCoded to bank 4 for data's initial values
		;mvn     #$05,#^_BEG_DATA ; Copy bytes		
		SKIP2:
...
N_COP   DW   ($8000+DIRQ)
N_BRK   DW   ($8000+DIRQ)
N_ABORT DW   ($8000+DIRQ)        
N_NMI   DW   ($8000+IRQ)
N_RSRVD DW   ($8000+DIRQ)
N_IRQ   DW   ($8000+DIRQ)
        DS   4
E_COP   DW   ($8000+DIRQ)
E_RSRVD DW   ($8000+DIRQ)
E_ABORT DW   ($8000+DIRQ)
E_NMI   DW   ($8000+DIRQ)
E_RESET DW   $8000
E_IRQ   DW   ($8000+DIRQ)
