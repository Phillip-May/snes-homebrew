...
def generate_spc_data(basepath: Path, sfx_list, music_list, music_raw):
    """Generate compact data tables for the SPC700 sequencer (p8_music_data.h)."""
    import os
    spc_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "spc700")
    out_path = os.path.join(spc_dir, "p8_music_data.h")
...
def main():
    ...
    try:
        ...
        # Generate SPC700 data tables (uses full sfx_list for correct indexing)
        music_raw_data = game.music.to_bytes()
        generate_spc_data(cart_path, sfx_list, music_list, music_raw_data)
