...
def generate_c_header(
    ...
    try:
        ...
        with open(output_path, 'w') as f:
            ...
            f.write(f"/* Tiles: {tiles_x}x{tiles_y} ({num_tiles} tiles, {len(tiles_data)} bytes) */\n")
            ...
            # Tile data array
            f.write(f"/* {var_base} Tile data - {len(tiles_data)} bytes ({num_tiles} tiles) */\n")
            ...
            # Constants
            f.write("/* Constants */\n")
            f.write(f"#define {var_base.upper()}TILES_COUNT {num_tiles}\n")
