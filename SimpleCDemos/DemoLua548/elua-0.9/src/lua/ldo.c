...
void luaD_throw (lua_State *L, int errcode) {
  ...
  if (L->errorJmp) {
    ...
    /* For SNES, don't use longjmp - just set the error status and return */
    /* This avoids the jump to NULL issue */
    return;
  }
  else {
...
int luaD_rawrunprotected (lua_State *L, Pfunc f, void *ud) {
  ...
  /* For SNES, use simple error handling without setjmp/longjmp */
  /* This avoids the problematic jump to NULL issue */
  L->errorJmp = &lj;
  
  /* Call the function directly - if it fails, we'll handle it gracefully */
  (*f)(L, ud);
...
int luaD_precall (lua_State *L, StkId func, int nresults) {
  ...
  if (cl && !cl->isC) {  /* Lua function? prepare its call */
    ...
    /* Check if we need to grow the CallInfo array before incrementing */
    if (L->ci == L->end_ci) {
      /* Grow the CallInfo array - inline the growCI logic to avoid Calypsi issue */
      if (L->size_ci > LUAI_MAXCALLS) {
        luaD_throw(L, LUA_ERRERR);
      } else {
        luaD_reallocCI(L, 2*L->size_ci);
