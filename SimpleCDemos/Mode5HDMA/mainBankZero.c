...
#include "initsnes.h"
#include "BG18_mode5.h" // Now contains Mode 1 data
#include "font_data.h"
...
// HDMA table is now defined in BG18_mode5.h
// The table hdma_bg12nba_table is generated by ConvertImageMode5.py

// Function to setup HDMA for BG1 tilemap base address updates
void setup_hdma_bg1_tilemap(void) {
	// Disable HDMA first
	REG_HDMAEN = 0x00;
	
	// Setup HDMA channel 7 for BG1 tilemap base address updates
	// DMAP7: 0x00 = 1 byte, direct addressing, increment source
	REG_DMAP7 = 0x00;
	
	// BBAD7: Destination register (BG12NBA = 0x210B)
	REG_BBAD7 = 0x0B;
	
	// A1T7: HDMA table address (low and high bytes)
	REG_A1T7 = (uint16_t)((uint32_t)hdma_bg12nba_table & 0xFFFF);
	
	// A1B7: HDMA table bank
	REG_A1B7 = (uint8_t)((uint32_t)hdma_bg12nba_table >> 16);
	
	// Enable HDMA channel 7
	REG_HDMAEN = 0x80; // Bit 7 = channel 7 enable
	
	// Debug: Set initial BG12NBA value to verify HDMA will change it
	REG_BG12NBA = 0x60; // Start with partition 0 (BG1 at 0x0000, BG2 at 0x6000)
}
...
void main(void){
	...
	REG_TM = 0x03; // Enable BG1 on main screen
	REG_TS = 0x03; // Enable BG1 on subscreen (should match TM)
	...
	// Use palette 7 for BG1 initialization
	...
	LoadVram((unsigned char *)bg_tiles_partition_0, 0x0000, sizeof(bg_tiles_partition_0));
	LoadVram((unsigned char *)bg_tiles_partition_1, 0x8000, sizeof(bg_tiles_partition_1)); // Load partition 1 at 0x2000

	LoadCGRam((unsigned char *)&font_palette, 0x00, sizeof(font_palette));
	LoadCGRam((unsigned char *)&bg_palette, 0x70, sizeof(bg_palette));
	LoadVram((unsigned char *)font_tiles, 0xC000, sizeof(font_tiles));
	...
	REG_BG2SC = 0x76; // Screen base address word 0x7400 (byte 0xE800), 32x32 tilemap
	REG_BG12NBA = 0x60; // BG1 at 0x0000, BG2 at 0xC000 (word address 0x6000)
	...
	// Setup HDMA for BG1 tilemap base address updates (after interrupt setup)
	setup_hdma_bg1_tilemap();
	...
	//emitCLI();
	REG_NMITIMEN = 0x80; // Enable NMI (bit 7)	
