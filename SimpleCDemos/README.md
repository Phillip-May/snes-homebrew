...
C data at build time, writing a gitignored `build/assets/*.inc` that the demo
`#include`s. Each such demo owns a `convert.py` that maps its art to outputs via
the `pySnesDevTools` library (repo root); its `Makefile` sets `ASSETS` and runs
`convert.py` as a build prerequisite. See `Mode1Sprite/convert.py` for the
pattern and `../pySnesDevTools/README.md` for the library/CLI.
...
Builds that currently succeed here (`build-all.ps1`: 29/50 pass):
...
| Mode1Platformer    | calypsi, vbcc65816, tcc816                   |
| BouncingSprites    | calypsi, llvm-mos, vbcc65816, tcc816         |
| DemoLua548         | calypsi                                      |
